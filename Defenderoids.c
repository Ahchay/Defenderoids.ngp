...
void DefenderoidsMain()
{
	...
	while (1)
	{
		...
		// Line version...
		/*
		iStartX=DefenderoidShip[0].x;
		iStartY=DefenderoidShip[0].y;
		iPoint = 0;
		...
		while (iPoint++<4)
		{
			...
			// Start with a border
			DrawLine((u16*)RugBitmap,(u8)((iStartX)*127),(u8)((iStartY)*127),(u8)((DefenderoidShip[iPoint].x)*127),(u8)((DefenderoidShip[iPoint].y)*127),1);

			// Scale is relatively easy
			DrawLine((u16*)RugBitmap,(u8)(1+(iStartX)*iScale),(u8)(1+(iStartY)*iScale),(u8)(1+(DefenderoidShip[iPoint].x)*iScale),(u8)(1+(DefenderoidShip[iPoint].y)*iScale),2);
			...
			iEndX = (DefenderoidShip[iPoint].x<<3)-16;
			iEndY = (DefenderoidShip[iPoint].y<<3)-16;
			...
			iStartX = DefenderoidShip[iPoint].x;
			iStartY = DefenderoidShip[iPoint].y;
		...
		*/
		...
		while (iPoint++<11)
		{
			...
			iStartX = (PrettyPattern[iPoint].x<<1)-8;
			iStartY = (PrettyPattern[iPoint].y<<1)-8;
			...
			SetPixel((u16*)RugBitmap,(u8)(1+((126-16)>>1)+(iStartX)),(u8)(1+((126-16)>>1)+(iStartY)),PrettyPattern[iPoint].colour);
